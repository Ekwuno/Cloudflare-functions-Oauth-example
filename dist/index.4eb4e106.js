...
function handleCode() {
    ...
    else throw new Error("No code found in url");
    async function sendCodeToServer() {
        const server = "https://test-functions.cloudflare-functions-oauth-example.pages.dev/api/code";
        try {
...
async function fetchRepos() {
    const server = "https://test-functions.cloudflare-functions-oauth-example.pages.dev/api/repos";
    try {
        const res = await fetch(server, {
            headers: {
                Authorization: `Bearer ${localStorage.getItem("jwt")}`
...
exports.default = {
    ...
    REDIRECT_URL: "https://test-functions.cloudflare-functions-oauth-example.pages.dev/",
...
},{}]},["eCWdY","frMjV"], "frMjV", "parcelRequiree026")
